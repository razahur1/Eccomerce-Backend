...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.fetchProducts = async function (page = 1, limit = 3) {
    currentPage = page;
    try {
      showSpinner();
      ...
      if (result.success) {
        ...
        updatePaginationControls(result.totalProducts);
      } else {
  ...
  function updatePaginationControls(totalProducts) {
    ...
    if (currentPage > 1) {
      ...
      prevLi.innerHTML = `<a class="page-link" href="javascript:void(0);" onclick="fetchProducts(${
        currentPage - 1
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      li.innerHTML = `<a class="page-link" href="javascript:void(0);" onclick="fetchProducts(${i}); return false;">${i}</a>`;
    ...
    if (currentPage < totalPages) {
      ...
      nextLi.innerHTML = `<a class="page-link" href="javascript:void(0);" onclick="fetchProducts(${
        currentPage + 1
      }); return false;">Next</a>`;
  ...
  window.editProduct = async (id, page) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const introDescription = getElement("intro-description");
        const detailDescription = getElement("detail-description");
        const priceInput = getElement("price");
        const genderInput = getElement("gender");
        const salePriceInput = getElement("sale-price");
        const saleDurationInput = getElement("sale-duration");
        const categoriesWrapper = getElement("categories-wrapper");
        const tagsInput = getElement("tags"); // Assuming you have an input for tags

        // Assign values to form fields if elements exist
        if (productId) productId.value = product._id || "";
        if (productName) productName.value = product.name || "";
        if (introDescription) introDescription.value = product.intro || "";
        if (detailDescription)
          detailDescription.value = product.description || "";
        quill.root.innerHTML = product.description;

        if (priceInput) priceInput.value = product.price || "";
        if (genderInput) genderInput.value = product.gender || "Men";
        if (salePriceInput) salePriceInput.value = product.salePrice || "";

        if (saleDurationInput) {
          saleDurationInput.value = product.saleEndDate
            ? new Date(product.saleEndDate).toISOString().slice(0, 16)
            : "";
        }
        if (tagsInput) {
          tagsInput.value = product.tags.join(", ") || ""; // Convert array to string
        }
        ...
        if (categoriesWrapper) {
          ...
          if (Array.isArray(product.category) && product.category.length) {
            product.category.forEach(({ _id: categoryId }) => {
              const categoryDiv = document.createElement("div");
              categoryDiv.className =
                "category-select-wrapper mb-2 d-flex align-items-center";
            ...
            console.warn("No categories found for this product.");
          }
        }

        const sizes = product.sizes || [];
        sizes.forEach(({ size, stock }) => {
          const stockInput = getElement(`stock-${size}`); // Ensure it matches the ID format
          if (stockInput) {
            stockInput.value = stock || 0; // Ensure stock is displayed
          }
        });

        Object.keys(product.highlights).forEach((key) => {
          const highlightCheckbox = getElement(`highlight-${key}`);
          if (highlightCheckbox) {
            highlightCheckbox.checked = product.highlights[key] || false;
          }
        });
